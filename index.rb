...
class TextProcessor #:nodoc:
  attr_reader :content_arr, :options, :final_result
  def initialize(content = nil, options = {})
    @options = options
    @content_arr = content.gsub("\n\n", '<br/>').split(' ')
    @final_result = finalize_work
  end
  ...
  def finalize_work
    ...
    subcontent = content.join(' ').gsub('<br/>', "\n\n")
    subcontent.split("\n\n")
  end

  def process_content
    options.each do |v|
      if content_arr[v[:start]] == '<br/>' && v[:start] + 1 < content_arr.length
        v[:start] += 1
      elsif content_arr[v[:end]] == '<br/>' && v[:end] + 1 < content_arr.length
        v[:end] += 1
      end
      background_color = generate_random_color_hex
      content_arr[v[:start]]
      .prepend('<span class="highlight" style="background-color:'+"#{background_color}"+ '" data-toggle="tooltip" title="'+"#{v[:comment]}"'">')
      content_arr[v[:end]].concat('</span>')
    end
    content_arr
  end

  def generate_random_color_hex
    color = (Random.new.bytes(3).unpack("H*")[0]).prepend('#')
    color
  end
end


class TextView #:nodoc:
  attr_reader :content, :options
  def initialize(content = '', options = {})
    @content = TextProcessor.new(content, options).final_result
...
#uncomment below if you would like to run the example
...
TextView.new(content, highlights).render